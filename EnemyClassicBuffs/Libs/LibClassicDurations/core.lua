...
local MAJOR, MINOR = "EnemyClassicBuffs-LibClassicDurations", 1
...
lib.scanTip = lib.scanTip or CreateFrame("GameTooltip", "EnemyClassicBuffsScanTip", nil, "GameTooltipTemplate")
...
local function RegenerateBuffList(unit, dstGUID)
    ...
    for i=1, 32 do
        ...
        spellName = EnemyClassicBuffsScanTipTextLeft1:GetText()
        if spellName then
...
-- Return only buffs reconstructed from combat-log events.  This deliberately
-- does not fall back to UnitAura, so consumers can display the hidden/tracked
-- list separately without duplicating buffs that the client still exposes.
function lib:GetTrackedEnemyBuffs(unit)
    if not enableEnemyBuffTracking
        or not unit
        or UnitIsFriend("player", unit)
    then
        return
    end
    ...
    return buffCache[unitGUID]
end

-- Return a combat-log-only snapshot for either BUFF or DEBUFF. This is kept
-- separate from UnitAura so callers can merge it with Blizzard's live data.
function lib:GetTrackedEnemyAuras(unit, requestedAuraType)
    if not enableEnemyBuffTracking
        or not unit
        or (
    ...
    local guidTable = dstGUID and guids[dstGUID]
    if not guidTable then
        return {}
    end

    local results = {}
    local function AddAura(spellID, auraTable)
        if auraTable and auraTable[3] == requestedAuraType then
            local info = makeBuffInfo(spellID, auraTable, dstGUID, nil)
            if info then
    ...
    for spellID, spellTable in pairs(guidTable) do
        if spellTable.applications then
            for _, auraTable in pairs(spellTable.applications) do
                AddAura(spellID, auraTable)
            end
        else
            AddAura(spellID, spellTable)
        end
    end
